...
// {/* top */}
// <div className="bg-white dark:bg-[#1D2226] rounded-lg overflow-hidden relative flex flex-col items-center text-center border border-gray-400 dark:border-none ">
...
//     <h4 className='hover:underline decoration-pink-600 underline-offset-1 cursor-pointer'>{session?.user?.name}</h4>
//     <p className='hover:underline decoration-purple-500 underline-offset-2 cursor-pointer'>{session?.user?.email}</p>
//    </div>
  ...
  return (
    <div className="space-y-2 min-w-max max-w-lg">
      ...
      <div className="bg-white dark:bg-[#1D2226] rounded-lg overflow-hidden relative flex flex-col items-center text-center border border-gray-300 dark:border-none">
        <div className="relative w-full h-14">
          <Image src="https://images.fastcompany.net/image/upload/w_596,c_limit,q_auto:best,f_auto/wp-cms/uploads/2021/03/LinkedIn-Default-Background-2020-.jpg" layout="fill" priority />
        </div>
        <Avatar
          onClick={signOut}
          src={session?.user?.image}
